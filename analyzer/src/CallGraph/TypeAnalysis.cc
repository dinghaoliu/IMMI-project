...
bool CallGraphPass::findCalleesWithMLTA(CallInst *CI, FuncSet &FS) {
    ...
    if (FS1.size() == 0) {
        ...
        Ctx->Global_MLTA_Result_Map[CI] = MissingBaseType;
        return false;
    }
    ...
    if(layer_result){
        ...
        if(TAResultSet.size() == 1){
            ...
            Ctx->Global_MLTA_Result_Map[CI] = TAResult;
            ...
            Ctx->Global_MLTA_Result_Map[CI] = TypeEscape;
        }
        else{
            Ctx->Global_MLTA_Result_Map[CI] = MixedLayer;
            FS1 = FS2;
        }
        ...
        if(!CTSet.empty()){
            ...
            if(!mergeFS.empty()){
                Ctx->Global_MLTA_Result_Map[CI] = TwoLayer;
                FS1 = mergeFS;
            }
            else{
                if(isTransEscape == 0){
                    Ctx->Global_MLTA_Result_Map[CI] = NoTwoLayerInfo;
                    FS1.clear();
                }
                else{
                    Ctx->Global_MLTA_Result_Map[CI] = TypeEscape;
            ...
            Ctx->Global_MLTA_Result_Map[CI] = OneLayer;
        }
    ...
    //Record if current icall analysis result
    if(LayerNo > 1){
        Ctx->valid_icallNumber++; // incorrect
        Ctx->valid_icallTargets+=FS.size();
        moduleIcallTargetsMap[current_module->getName().str()] += FS.size();
    }
