...
{
    logger->info("start escapeHandler");
    ...
    logger->info("related types: {0}", hash_num);
    logger->info("succeed_type_num: {0}", succeed_type_num);
    logger->info("related SI_num: {0}", SI_num);
    logger->info("succeed_inst_num: {0}", succeed_inst_num);

    for(auto it = Ctx->Global_MLTA_Result_Map.begin(); it != Ctx->Global_MLTA_Result_Map.end(); it++){
    ...
    while (!EV.empty()) {
        ...
        if(CAI){
            ...
            if(FName == "__symbol_get"){
                string symbol_str = symbol_get_handler(CAI);
