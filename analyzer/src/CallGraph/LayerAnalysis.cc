...
bool CallGraphPass::nextLayerBaseType_new(Value *V, list<CompositeType> &TyList, 
    ...
    //For only type analysis, we will drop V's further info if V is an argument, 
    ...
    // UnaryInstruction includes castinst, load, etc, resolve this in the last step
        ...
        // zpl todo: other types of instructions that may need to be account.
        // logger->warn("unknown inst in nextLayerBaseType_new: {0}", common::llobj_to_string(V)); 
        return false;
    }
}
