...
set<Instruction *>UAFCheckerPass::ErrSelectInstSet;

ICmpInst *isUnlikely(ICmpInst* ICI){
    
    if(!ICI)
        return NULL;
    
    Instruction* pre_1 = ICI->getPrevNonDebugInstruction();
    if(!pre_1)
        return NULL;
    
    if(pre_1->getOpcode() != Instruction::SExt)
        return NULL;
    ...
    if(pre_2->getOpcode() != Instruction::ZExt)
        return NULL;
    ...
    if(pre_3->getOpcode() != Instruction::Xor)
        return NULL;
    
    Instruction* pre_4 = pre_3->getPrevNonDebugInstruction();
    if(!pre_4)
        return NULL;
    
    if(pre_4->getOpcode() != Instruction::Xor)
        return NULL;
    
    Instruction* pre_5 = pre_4->getPrevNonDebugInstruction();
    if(!pre_5)
        return NULL;
    
    ICmpInst *ici = dyn_cast<ICmpInst>(pre_5);
    if(!ici)
        return NULL;
...
void UAFCheckerPass::checkErrReturn(Function *F, BBErrMap &bbErrMap,
...
void UAFCheckerPass::checkErrValueFlow(
        Function *F,
        ReturnInst *RI, 
        std::set<Value *> &PV, 
        BBErrMap &bbErrMap,
            ...
            if(CallBB->getTerminator()->getNumSuccessors() > 1){
                ...
                if (Cond){
                    ...
                    if(ICI){
                        ...
                        //Check unlikely macro
                        ICmpInst *pre_ici = isUnlikely(ICI);
                        if(pre_ici){
                            oprand0 = pre_ici->getOperand(0);
                            oprand1 = pre_ici->getOperand(1);
                        }

                        Instruction *I_tar = CaI;
                        for(User *U : CaI->users()){
                            if (ICI == U)
                                continue;
                            StoreInst *SI = dyn_cast<StoreInst>(U);
                            if(SI){
                                auto pop = SI->getPointerOperand();
                                for(User *U2 : pop->users()){
                                    if(U2 == SI)
                                        continue;
                                    if(auto LI2 = dyn_cast<LoadInst>(U2)){
                                        if(LI2->getParent() != CallBB)
                                            continue;
                                        I_tar = LI2;
                                        break;
                                    }
                        ...
                        if(isConstant(oprand0) && oprand1 == I_tar){
                            auto opconst = dyn_cast<Constant>(oprand0);
                            if(opconst->isNullValue()){
                                blockAttributeMap[I_tar->getParent()]=Cond;
                            }
                            
                        }
                        else if(isConstant(oprand1) && oprand0 == I_tar){
...
bool UAFCheckerPass::isValueErrno(Value *V, Function *F) {
    // Invalid input.
    if (!V)
        return false;
...
void UAFCheckerPass::markBBErr(BasicBlock *BB, 
...
void UAFCheckerPass::findSameVariablesFrom(Value *V, 
        std::set<Value *> &VSet) {
...
/// error
bool UAFCheckerPass::mayReturnErr(Function *F) {
...
int UAFCheckerPass::inferErrBranch(Instruction *Cond) {
...
void UAFCheckerPass::addSelfLoopEdges(Function *F,
...
bool UAFCheckerPass::markAllEdgesErrFlag(Function *F, BBErrMap &bbErrMap, 
...
/// closest branches
void UAFCheckerPass::recurMarkEdgesToErrHandle(BasicBlock *BB, 
        EdgeErrMap &edgeErrMap) {
    // Invalid input.
    if (!BB)
        return;
...
/// Incorporate flags
void UAFCheckerPass::updateReturnFlag(int &errFlag, int &newFlag) {
...
void UAFCheckerPass::recurMarkEdgesToBlock(CFGEdge &CE, int flag, 
...
void UAFCheckerPass::recurMarkEdgesFromBlock(CFGEdge &CE, int flag, 
        BBErrMap &bbErrMap, EdgeErrMap &edgeErrMap) {
...
void UAFCheckerPass::recurMarkEdgesToErrReturn(BasicBlock *BB, 
...
void UAFCheckerPass::mergeFlag(int &errFlag, int &newFlag) {
...
void UAFCheckerPass::updateHandleFlag(int &errFlag, int &newFlag) {
...
void UAFCheckerPass::markCallCases(Function *F,Value * Cond, EdgeErrMap &edgeErrMap){
...
bool UAFCheckerPass::checkEdgeErr(CFGEdge edge, EdgeErrMap edgeErrMap){
...
void UAFCheckerPass::dumpErrEdges(EdgeErrMap &edgeErrMap) {
...
void UAFCheckerPass::recurMarkErrEdges(EdgeErrMap &errEdgeMap){

    EdgeErrMap addition_edges;
    list<CFGEdge> to_analyze_edges;
    set<CFGEdge> analyzed_edges;

    for (auto it = errEdgeMap.begin(); it != errEdgeMap.end(); ++it) {
        CFGEdge edge = it->first;
        int flag = it->second;

        to_analyze_edges.push_back(edge);
    }

    while(!to_analyze_edges.empty()){
        
        CFGEdge TE = to_analyze_edges.front();
        to_analyze_edges.pop_front();
        if (analyzed_edges.count(TE) != 0)
            continue;
        analyzed_edges.insert(TE);

        pair<CFGEdge,int> value(TE,1);
        errEdgeMap.insert(value);

        BasicBlock* next_front_bb = TE.second;
        Instruction *TI =next_front_bb->getTerminator();
        for(BasicBlock *Succ : successors(next_front_bb)){
            CFGEdge new_edge = make_pair(TI,Succ);
            to_analyze_edges.push_back(new_edge);
        }
    }
    
    to_analyze_edges.clear();
    analyzed_edges.clear();
    set<BasicBlock*> headset;
    set<BasicBlock*> endset;
    for (auto it = errEdgeMap.begin(); it != errEdgeMap.end(); ++it) {
        CFGEdge edge = it->first;
        int flag = it->second;
        ...
        BasicBlock* headbb = edge.first->getParent();
        BasicBlock* endbb = edge.second;
        headset.insert(headbb);
        endset.insert(endbb);
    }
    ...
    while(!to_analyze_edges.empty()){
        ...
        BasicBlock* headbb = TE.first->getParent();
        BasicBlock* endbb = TE.second;
        headset.insert(headbb);
        endset.insert(endbb);

        if(endset.count(headbb))
            continue;
        
        bool all_err = true;
        for(BasicBlock *Succ : successors(headbb)){
            if(Succ == headbb)
                continue;
            CFGEdge Edge = std::make_pair(headbb->getTerminator(), Succ);
            if(errEdgeMap.count(Edge) == 0){
                all_err = false;
                break;
            }
        }
        ...
        for (BasicBlock *Pred : predecessors(headbb)) {
            if(Pred == headbb)
                continue;
            Instruction *TI =Pred->getTerminator();
            CFGEdge new_edge = make_pair(TI,headbb);
            to_analyze_edges.push_back(new_edge);
        }
    }
}